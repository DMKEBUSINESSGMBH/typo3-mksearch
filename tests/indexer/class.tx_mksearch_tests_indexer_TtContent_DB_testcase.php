...
//		$this->assertEquals(true, $indexDoc->getDeleted(), 'Wrong deleted state for uid '.$record['uid']);
	 ...
	 */
	public function test_prepareSearchData_CheckIncludePageTrees() {
		$indexer = new tx_mksearch_indexer_TtContent();
		list($extKey, $cType) = $indexer->getContentType();
	 ...
	 * angegebenen Seitenbaum liegen
	 */
	public function test_prepareSearchData_CheckExcludePageTrees() {
		$indexer = new tx_mksearch_indexer_TtContent();
		list($extKey, $cType) = $indexer->getContentType();
	...
	public function testPrepareSearchDataPutsCorrectElementsIntoTheQueueIfTablePages() {
		...
		$this->assertEquals('tt_content',$aResult[0]['tablename'],'Es wurde nicht das richtige Element (tablename) in die queue gelegt!');
		$this->assertEquals(1,$aResult[0]['recid'],'Es wurde nicht das richtige Element (recid) in die queue gelegt!');
		$this->assertEquals('tt_content',$aResult[1]['tablename'],'Es wurde nicht das richtige Element (tablename) in die queue gelegt!');
		$this->assertEquals(2,$aResult[1]['recid'],'Es wurde nicht das richtige Element (recid) in die queue gelegt!');
	}
